...
describe("summarizeStreamEntries", () => {
  it("collects tasks, acknowledgement ids, and message groupings", () => {
    ...
    expect(summary).toEqual({
      ...
      tasks: ["task-1", "task-2"],
      messages: [
        { ackId: "1-0", tasks: ["task-1"] },
        { ackId: "2-0", tasks: ["task-2"] },
      ],
    });
  });

  it("acknowledges entries without tasks while leaving their task lists empty", () => {
    const summary = summarizeStreamEntries(
      {
        "3-0": [["note", "value"]],
        "4-0": null,
